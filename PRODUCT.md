...
integrity-screened shortlist they can act on and export — deterministic, CPU-only, in seconds.*
...
This is why our shipped ranking is the **integrity-safe hedge** — it excludes egregious impossible-tenure
candidates rather than chasing a higher score on them, and an independent recruiter's labels confirmed
that choice (`docs/external_recruiter_validation.md`). **Our top-100 ships 0 honeypots.**
...
- **Production-shaped:** deterministic, CPU-only, <5 min on 100K, offline — scales to a 200K pool
