...
| API — desktop | API — mobile |
|:---:|:---:|
| ![API desktop](docs/assets/api-desktop.png) | ![API mobile](docs/assets/api-mobile.png) |
| 100K-pool evidence, all eight pipeline stages, shortlist and audit detail | Responsive one-column evidence flow with touch-safe controls |
...
reproducibility, integrity, and validation, **not** on these tie-breaks. `dominant-v4` (the same
pipeline without the two tie-breaks) is available as a profile for anyone who wants to compare.
...
| Study | Result |
|---|---|
| full evaluator matrix vs `main` | wins all 15 composite evaluators; vs `dominant-v4`: 6 wins / 54 ties / 0 losses across 60 component cells |
