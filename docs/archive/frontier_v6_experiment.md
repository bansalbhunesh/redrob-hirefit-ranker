...
Final suite after battle-proof hardening and positioning: 268 collected, 262 passed, 6 environment skips.
...
The final gate was exercised inside the pinned image with `--cpus=2
--memory=16g --workers 2`: 100,000 loaded and ranked, 53 honeypots detected,
zero emitted, **136.0 s pipeline / 149.1 s wall**, exit code 0, zero output
temps, and the exact golden SHA-256. The output was published only after verification.
...
and leaving zero mounted temporary files, confirming atomic failure behavior.
Full main-versus-champion evidence is in
