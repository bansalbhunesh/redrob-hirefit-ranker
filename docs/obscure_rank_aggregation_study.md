...
> **SHIPPED ON THIS BRANCH (2026-06-16).** Per the user's (B)-bet decision, `submission.csv` on
> `research/obscure-rank-aggregation` is now the **Copeland** ranking (sha256 `d0372aed…`),
> replacing golden `af8f2b32`. Verified: full suite **198 passed, 0 skipped**; official validator
> passes; `reproduce.sh` green; the production pipeline (`rank.py`) is **unchanged** and still
> reproduces the golden baseline byte-for-byte (slice gate green) — Copeland is a deterministic
> post-hoc reranking of its top-3000 pool, regenerated by `experiments/build_copeland_submission.py`.
> Hash pins updated in lockstep (submission gate, firewall test, smoke test, metrics manifest,
> dashboard constants, reproduce.sh). **`main` keeps golden `af8f2b32` frozen and untouched** — this
> ship lives only on the branch until a deliberate merge. It is the anachronism-promotion bet:
> wins iff the hidden labels do not date-check technology tenure. Composite 0.8779 vs golden 0.8625
> on the blind arbiter; 7/7 label sets; nested R=20 +0.0142 (19/20); NDCG@10 held at 0.8288;
> 0 shipped-honeypot flags in the top-100.
