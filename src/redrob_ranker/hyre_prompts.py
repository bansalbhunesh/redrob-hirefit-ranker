...
def _hyre_token_set(jd_text: str) -> frozenset:
    """Token set of the role template for a JD.

    On the default ranking path `jd_text` is a single constant, so without this
    cache the identical template was re-tokenized once per candidate (100K times
    on the full pool). Pure function of the input -> byte-identical output.
    """
    hyre_text = get_hyre_for_role(jd_text)
    return frozenset(hyre_text.lower().replace(",", " ").replace(".", " ").split())
